...
try {
  ...
  // --- 用例14：触屏最小持按保底（点按不该被当成"我要短跳"） ---
  function jumpPeak(src, releaseAt) {
    G.startLevel(0);
    const p = G.player(), inp = G.input();
    inp.left = inp.right = inp.down = false; inp.jump = false; inp.jumpQueued = 0;
    for (let i = 0; i < 10; i++) G.update(1 / 60);          // 落地稳定
    if (!p.onGround) return -1;
    const y0 = p.y; let minY = p.y;
    inp.jumpSrc = src; inp.jump = true; inp.jumpQueued = 0.12;
    for (let i = 0; i < 90; i++) {
      if (i === releaseAt) { inp.jump = false; inp.jumpQueued = 0; }   // 抬手
      G.update(1 / 60);
      if (p.y < minY) minY = p.y;
      if (i > 4 && p.onGround) break;
    }
    return (y0 - minY) / 32;                                 // 峰值高度（格）
  }
  const tapTouch = jumpPeak('touch', 5), tapFast = jumpPeak('touch', 4);
  const tapKey = jumpPeak('key', 5), holdKey = jumpPeak('key', 9999);
  check('触屏点按跳(83ms) ≥2.6 格（实测 ' + tapTouch.toFixed(2) + '）', tapTouch >= 2.6);
  check('触屏快点按跳(67ms) 也过 2 格管（实测 ' + tapFast.toFixed(2) + '）', tapFast >= 2.5);
  check('键盘同长度点按仍是短跳（实测 ' + tapKey.toFixed(2) + '）', tapKey < 2.4 && tapTouch - tapKey > 0.4);
  check('长按到顶仍 ≈4 格（实测 ' + holdKey.toFixed(2) + '）', holdKey > 3.7 && holdKey < 4.3);

  // --- 用例15：冲刺跳的距离收益（跑键值得单独占一个指位的前提） ---
  function jumpDistance(run){
    G.startLevel(0);
    const p = G.player(), inp = G.input();
    inp.jump = false; inp.jumpQueued = 0; inp.left = false; inp.down = false;
    inp.right = true; inp.run = run; inp.jumpSrc = 'key';
    for (let i = 0; i < 60 * 2; i++) { G.update(1 / 60); if (Math.abs(p.vx) >= (run ? 430 : 240)) break; }  // 先加速到档
    const x0 = p.x;
    inp.jump = true; inp.jumpQueued = 0.12;
    for (let i = 0; i < 200; i++) {
      G.update(1 / 60);
      if (i > 6 && p.onGround) break;                     // 落地即止
      if (p.y < 0) break;
    }
    inp.jump = false; inp.right = false; inp.run = false;
    return (p.x - x0) / 32;                               // 水平跨越（格）
  }
  const dWalk = jumpDistance(false), dRun = jumpDistance(true);
  check('走跳跨越 ' + dWalk.toFixed(1) + ' 格', dWalk > 3);
  check('冲刺跳跨越 ' + dRun.toFixed(1) + ' 格 ≥ 走跳 1.5 倍', dRun >= dWalk * 1.5);
} catch (e) {
